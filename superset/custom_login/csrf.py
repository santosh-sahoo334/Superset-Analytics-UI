...
def generate_csrf(secret_key=None, token_key=None):
    ...
    if field_name not in g:
        ...
        if field_name not in session:
            print(f"Inside - generate_csrf :: Field Name {field_name} not in Session.....Setting it now...")
            session[field_name] = hashlib.sha1(os.urandom(64)).hexdigest()
            session_field_name = session[field_name]
            print(f"Inside - generate_csrf :: After setting the field name - {session_field_name}")
