...
class AuthOAuthView(AuthView):
    ...
    def csight_oauth_authorized(self) -> WerkzeugResponse:
        ...
        try:
            ...
            log.warning("[oauth_callback] userinfo: %s", userinfo)
            user = self.appbuilder.sm.auth_user_oauth(userinfo)
            log.warning("[oauth_callback] auth_user_oauth returned: %s", user)
            if user is None:
                flash(as_unicode(self.invalid_login_message), "warning")
                return redirect(f"https://{host}/login/")

            roles_before = [r.name for r in user.roles] if user.roles else []
            log.warning("[oauth_callback] user.roles BEFORE login_user: %s", roles_before)
